...
### 2. Автоматическое подключение агентов

#### Вариант А: Запуск в одну строку (Без скачивания репозитория!)
Выполните прямо в терминале на любом удаленном ноутбуке или рабочей станции:
- **Windows (PowerShell)**:
  ```powershell
  irm http://<IP_СЕРВЕРА>:8200/install.ps1 | iex
  ```
- **macOS и Linux (Bash)**:
  ```bash
  curl -sSL http://<IP_СЕРВЕРА>:8200/install.sh | bash
  ```
- **Прямой standalone-скрипт**: `http://<IP_СЕРВЕРА>:8200/scanner.py` (только стандартная библиотека Python, 0 внешних зависимостей).

#### Вариант Б: Из клонированного репозитория
- **macOS и Linux**:
  ```bash
  ./connect-agents.sh
  # Либо с указанием удаленного сервера:
  ./connect-agents.sh http://<IP_СЕРВЕРА>:8200/sse <ВАШ_AUTH_TOKEN>
  ```
- **Windows (PowerShell)**:
  ```powershell
  .\connect-agents.ps1
  # Либо с указанием удаленного сервера:
  .\connect-agents.ps1 -Url "http://<IP_СЕРВЕРА>:8200/sse" -Token "<ВАШ_AUTH_TOKEN>"
  ```
- **Либо через Веб-интерфейс**: откройте `http://<IP_СЕРВЕРА>:8200` ➔ раздел **«Флот и сканер»** ➔ нажмите **«Найти и подключить агентов»**.
...
В репозиторий включен полный тестовый набор (векторные вычисления FastEmbed, FLCR версионирование, репликация навыков и MCP-серверов, обработчики JSON-RPC, трекер флота, инъектор сканера и коллекторы демона):
...
- **[Справочник API: REST & MCP](docs/api-reference.ru.md)** ([English version](docs/api-reference.md)): Полная спецификация эндпоинтов, инструментов FLCR, навыков, MCP Registry и форматов JSON-RPC.
...
### 🟢 Выполнено (v0.1.0 – v0.3.0)
- [x] Полноценный FastMCP SSE (Server-Sent Events) транспорт с Bearer-аутентификацией.
- [x] Векторная семантическая память на базе PostgreSQL 16 + pgvector (HNSW) и FastEmbed CPU.
- [x] Двуязычный Веб-Дашборд (RU / EN) и Live Fleet Tracker сессий агентов.
- [x] Автономный фоновый демон синхронизации истории чатов в `AGENTS.md`, `CLAUDE.md`, `.cursorrules`.
- [x] **Fact-Level Conflict Resolution (FLCR)**: версионирование атомарных фактов проекта (`v1 -> v2`), политики LWW и Authority Hierarchy (User > Lead > Worker), MCP-инструменты `fact_*`.
- [x] **Кросс-агентная репликация навыков (Skills Sync)**:
  - Единый канонический формат навыков (`CanonicalSkill`: `SKILL.md` + скрипты + шаблоны).
  - Адаптеры для переноса навыков между **Antigravity**, **Cursor IDE**, **Claude Code**, **OpenAI Codex** и **Windsurf**.
  - Инструменты MCP: `skill_publish`, `skill_list`, `skill_get`, `skill_install(target_agent)`.
- [x] **Централизованный реестр MCP-серверов (MCP Fleet Hub)**:
  - Единая база конфигураций проверенных MCP-серверов в PostgreSQL.
  - Установка и проброс любого MCP-сервера в локальные конфиги агентов (`mcp_server_install`).

### 🟡 В планах (v0.4.0)
- [ ] Визуальный конструктор и редактор навыков в Web UI.
- [ ] Семантическая детекция дубликатов навыков при публикации.
