...
### 🟡 В разработке (v0.3.0) — Cross-Agent Skills & MCP Replication
- [ ] **Кросс-агентная репликация навыков (Skills Sync)**:
  - Единый канонический формат навыков (`CanonicalSkill`: `SKILL.md` + скрипты + шаблоны).
  - Адаптеры для взаимного копирования навыков между **Antigravity**, **Cursor IDE**, **Claude Code**, **OpenAI Codex** и **Windsurf**.
  - MCP-инструменты агентов: `skill_publish`, `skill_search`, `skill_install(target_agents)`.
- [ ] **Централизованный реестр MCP-серверов (MCP Fleet Hub)**:
  - Единая база конфигураций проверенных MCP-серверов в PostgreSQL.
  - Установка и проброс любого MCP-сервера во все установленные агенты на машине в 1 клик.
  - Матрица соответствия серверов и агентов в веб-дашборде.
- [ ] **Автоматическое распространение через фоновый демон**:
  - Детекция нового навыка в любом агенте и авто-синхронизация по всему флоту.
